...
/**
 * Classe dell'istruzione JR con tutti i possibili indirizzamenti
 * @author Roberto Palmieri, Giulio Vennari
 */
public class JR extends Istruzione {
    /**
     ...
     */
    protected JR(LinkedList l, char[] c) throws ParseException {
		super(l.get(0), ((l.size() == 4) ? l.get(3) : l.get(2)), l.get(1));
        assegnaByte(l, c);
        comment.add("Instruction JR: IF F/F STATUS[device] == 1 then go to DEST");
    }
	...
	protected LinkedList getMO(){
		if(BYTE[3] == "10"){
			...
			mop.add("IF F/F STATUS[device] == 1 then");
