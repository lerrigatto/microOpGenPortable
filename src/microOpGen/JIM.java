...
/**
 * Classe dell'istruzione JIM con tutti i possibili indirizzamenti
 * @author Roberto Palmieri, Giulio Vennari
 */
public class JIM extends Istruzione {
    /**
     ...
     */
    protected JIM(LinkedList l, char[] c) throws ParseException {
		super(l.get(0), ((l.size() == 4) ? l.get(3) : l.get(2)), l.get(1));
        assegnaByte(l, c);
        comment.add("Instruction JIM: IF F/F IM[device] == 1 then go to DEST");
    }
	...
	protected LinkedList getMO(){
		if(BYTE[3] == "10"){
			...
			mop.add("IF F/F IM[device] == 1 then");
