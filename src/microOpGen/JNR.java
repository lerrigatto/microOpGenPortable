...
/**
 * Classe dell'istruzione JNR con tutti i possibili indirizzamenti
 * @author Roberto Palmieri, Giulio Vennari
 */
public class JNR extends Istruzione {
    /**
     ...
     */
    protected JNR(LinkedList l, char[] c) throws ParseException {
		super(l.get(0), ((l.size() == 4) ? l.get(3) : l.get(2)), l.get(1));
        assegnaByte(l, c);
        comment.add("Instruction JNR: IF F/F STATUS[device] == 0 then go to DEST");
    }
	...
	protected LinkedList getMO(){
		if(BYTE[3] == "10"){
			...
			mop.add("IF F/F STATUS[device] == 0 then");
